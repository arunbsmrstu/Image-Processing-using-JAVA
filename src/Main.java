...
import com.imageprocessing.image.OpenCVImage;
import com.imageprocessing.opencv.library.Library;
import com.imageprocessing.opencv.masking.Mask;
import com.imageprocessing.opencv.normalization.Normalization;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.HashSet;
import static jdk.nashorn.internal.objects.NativeMath.atan2;
...
//        skinMask.copyTo(currentSkinMask);
    ...
    public static void main(String[] args) {
        ...
        Mat mat= opencvIMage.getMatfromGrayScaleIMage("image/finger_saved.png");
        opencvIMage.saveImage("output/rot.png",mat);
        Mat equalized = new Mat(mat.rows(), mat.cols(), CvType.CV_32FC1);
        Imgproc.equalizeHist(mat, equalized);
        opencvIMage.saveImage("output/equalized.png",equalized);
        
        // convert to float, very important
        Mat floated = new Mat(mat.rows(), mat.cols(), CvType.CV_32FC1);
        equalized.convertTo(floated, CvType.CV_32FC1);
        opencvIMage.saveImage("output/ejjgj.png",equalized);
        
         Mat skeleton = m.getSkeletonImage(floated, mat.rows(), mat.cols());
         skeleton = m.thinning(skeleton);
         opencvIMage.saveImage("output/skeleton.png",skeleton);
         Mat skeleton_with_keypoints = m.detectMinutiae(skeleton, 1);
         opencvIMage.saveImage("output/skeleton_with_keypoints.png",skeleton_with_keypoints);
    }
}
